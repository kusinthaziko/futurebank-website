  ...
  return (
    <section ref={ref} className="py-20 px-6 max-w-5xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 32 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
        <p className="text-xs font-bold uppercase tracking-widest mb-3 text-[var(--blue-light)]">The Team</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {founders.map((f, i) => (
          <motion.div key={f.name} initial={{ opacity: 0, y: 32 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            ...
            className="rounded-3xl border border-[var(--border)] p-7 flex items-start gap-5 hover:-translate-y-1 hover:border-[var(--blue)] transition-all duration-200"
            style={{ background: "var(--card)" }}>
            <div className="w-14 h-14 rounded-2xl shrink-0 flex items-center justify-center text-2xl font-black text-[var(--blue-light)]"
              style={{ background: "rgba(26,86,219,0.2)" }}>{f.name[0]}</div>
            <div>
              <div className="font-bold text-lg">{f.name}</div>
              <div className="text-sm text-[var(--blue-light)] mb-2">{f.role}</div>
