  ...
  return (
    <section ref={ref} className="py-20 px-6 max-w-6xl mx-auto">
      <motion.div initial={{ opacity: 0, y: 32 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
        <p className="text-xs font-bold uppercase tracking-widest mb-3 text-[var(--teal-light)]">The Problem</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {pain.map((p, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 32 }} animate={inView ? { opacity: 1, y: 0 } : {}}
            ...
            className="rounded-3xl border border-[var(--border)] p-8 hover:-translate-y-1 hover:border-[var(--teal)] transition-all duration-200"
