...
const features = [
  { icon:"🤖", title:"AI Financial Coach", desc:"Personalised spending insights and financial guidance powered by AI.", wide:true,  bg:"rgba(0,105,92,0.15)" },
  ...
  { icon:"💳", title:"Micro-Loans",         desc:"Apply for campus loans in minutes.",                              wide:false, bg:"rgba(0,105,92,0.15)" },
];
  ...
  return (
    <section ref={ref} id="features" className="py-20 px-6 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {features.map((f, i) => (
          <motion.div key={f.title}
            ...
            className={cn("rounded-3xl border border-[var(--border)] p-7 transition-all duration-200 hover:-translate-y-1 hover:border-[var(--teal)]",
      ...
      <motion.div initial={{ opacity:0 }} animate={inView?{opacity:1}:{}} transition={{ delay:0.5 }}
        ...
        <Link href="/features"
          className="inline-flex items-center gap-2 px-7 py-4 rounded-full font-semibold text-[var(--muted)] border border-[var(--border)] transition-all hover:border-[var(--teal)] hover:text-white">
          See all features →
        </Link>
      </motion.div>
    </section>
